...
# =====================================================================
# 2. 100% FREE CONGRESSIONAL DATA LAYER (AWS S3 DATA LAKES)
# =====================================================================
def fetch_free_congressional_layer(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    """
    Pulls STOCK Act disclosures directly from open-source AWS S3 buckets.
    Applies robust date-parsing and extends the lookback window to capture 
    historical trades from legacy community datasets.
    """
    print(f"[+] Querying open-source AWS S3 Data Lakes for ${ticker.upper()}...")
    
    # Extend lookback to ensure legacy S3 buckets return data for the matrix
    political_lookback = max(lookback_days, 1500)
    cutoff_date_obj = datetime.now() - timedelta(days=political_lookback)
    ...
    for url in endpoints:
        repo_name = url.split('.com/')[0].split('//')[-1]
        try:
            ...
            for row in data:
                ...
                # Robust Date Parsing (Handles both YYYY-MM-DD and MM/DD/YYYY)
                raw_date = str(row.get("disclosure_date", row.get("transaction_date", "")))
                try:
                    if "/" in raw_date:
                ...
                # Apply cutoff filter
                if parsed_date < cutoff_date_obj:
                    continue
                ...
                chamber = "Senate" if "senate" in repo_name else "House"
            ...
            if match_count > 0:
                print(f"  [+] {chamber} Data Lake: Extracted {match_count} historical trades.")
    ...
    if records:
        ...
        print(f"[-] No political trades found for ${ticker.upper()} in the S3 archives.")
