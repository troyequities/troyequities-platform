...
# =====================================================================
# 2. CONGRESSIONAL DATA LAYER (FINNHUB)
# =====================================================================
def fetch_finnhub_congressional(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    """
    Pulls live Congressional trades using the Finnhub Alternative Data API.
    """
    print(f"[+] Querying Finnhub Congressional Trading API for ${ticker.upper()}...")
    ...
    try:
        ...
        # Finnhub returns data inside a "data" array
        data = res.json().get("data", [])
        
        records = []
        for row in data:
            # Get disclosure date (fallback to transaction date if missing)
            ...
            tx_type = str(row.get("type", "UNKNOWN")).upper()
            ...
            # Estimate dollar value from Finnhub's reported STOCK Act ranges
        ...
        if records:
            print(f"  [+] Extracted {len(records)} Finnhub political trades.")
        else:
            print(f"  [-] No political trades found for ${ticker.upper()} in this window.")
...
def fetch_finnhub_insider(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    ...
    Bypasses SEC XML parsing entirely.
    """
    ...
    try:
        ...
        records = []
        for row in data:
            # We want filingDate as it marks when the market received the info
            ...
            # Finnhub represents buying as positive share change, selling as negative
            ...
            # Filter out Option Grants / RSUs (which execute at a $0 price basis)
            if price <= 0:
                continue
...
def get_unified_flow_data(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    if not FINNHUB_API_KEY or FINNHUB_API_KEY == "YOUR_FINNHUB_API_KEY":
        print("\n[-] FATAL: Please insert your Finnhub API Key on Line 14.")
        return pd.DataFrame()
        
    pol_df = fetch_finnhub_congressional(ticker, lookback_days)
