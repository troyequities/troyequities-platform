...
# =====================================================================
# 2. 100% FREE CONGRESSIONAL DATA LAYER (OPEN-SOURCE GITHUB LAKES)
# =====================================================================
def fetch_free_congressional_layer(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    """
    Scrapes active U.S. House and Senate trading disclosures directly from public
    open-source GitHub data repositories, eliminating all API subscription fees.
    """
    cutoff_date = (datetime.now() - timedelta(days=lookback_days)).strftime("%Y-%m-%d")
    records = []

    # Community-maintained repositories that continuously scrape official government portals
    open_data_endpoints = [
    ...
    for endpoint in open_data_endpoints:
        try:
            res = requests.get(endpoint, headers=headers, timeout=10)
            if res.status_code == 200:
                data = res.json()
                for tx in data:
                    # Match ticker symbol across varying schema formatting
                    tx_ticker = str(tx.get("ticker", tx.get("symbol", ""))).strip().upper()
                    if tx_ticker != ticker.upper():
                        continue
                    
                    # Clean and filter by disclosure date
                    raw_date_str = str(tx.get("disclosure_date", tx.get("date", tx.get("transaction_date", tx.get("ReportDate", "")))))
                    raw_date = raw_date_str.split("T")[0]
                    if not raw_date or raw_date < cutoff_date:
                        continue
                    
                    # Normalize transaction types
                    raw_type = str(tx.get("type", tx.get("transaction", ""))).upper()
                    ...
                    # Extract lawmaker name and chamber
                    rep_name = tx.get("representative", tx.get("senator", tx.get("name", tx.get("first_name", "Lawmaker"))))
                    if "last_name" in tx:
                        rep_name = f"{tx.get('first_name', '')} {tx.get('last_name', '')}".strip()
                    chamber = tx.get("chamber", tx.get("house", "Congress")).title()
                    
                    # Estimate midpoint dollar value from STOCK Act disclosure ranges
                    val_str = str(tx.get("amount", tx.get("value", "15000")))
                    if "1,000,000" in val_str or "500,000" in val_str: val = 750000.0
                    elif "250,000" in val_str or "100,000" in val_str: val = 175000.0
                    elif "50,000" in val_str: val = 32500.0
                    else: val = 15000.0
                ...
                # Break loop if we successfully populated records from a primary lake
                if records:
                    break
        except Exception as e:
            continue  # Silently fall back to the next open-source repository if offline
