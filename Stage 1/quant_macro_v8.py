...
# =====================================================================
# 2. 100% FREE CONGRESSIONAL DATA LAYER (QUIVER OPEN BETA FEEDS)
# =====================================================================
def fetch_free_congressional_layer(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    """
    Scrapes live U.S. House and Senate trading disclosures using Quiver Quantitative's 
    free open beta JSON endpoints. Zero API key required.
    """
    print(f"[+] Querying open public political feeds for ${ticker.upper()}...")
    cutoff_date = (datetime.now() - timedelta(days=lookback_days)).strftime("%Y-%m-%d")
    
    # Quiver's open beta endpoints (No authentication required)
    endpoints = [
        ...
        try:
            ...
            if res.status_code == 200:
                ...
                for row in data:
                    ...
                    date_str = str(row.get("ReportDate", row.get("TransactionDate", "")))[:10]
                    if not date_str or date_str < cutoff_date:
                        continue
                    
                    # Extract entity name and chamber
                    entity = str(row.get("Representative", row.get("politician", "Lawmaker"))).strip()
                    chamber = str(row.get("House", "Congress")).capitalize()
                    
                    # Normalize Buy/Sell signal
                    tx_type = str(row.get("Transaction", row.get("type", ""))).upper()
                    ...
                    # Estimate dollar value from reported ranges
                    val_str = str(row.get("Amount", row.get("Range", "15000"))).replace("$", "").replace(",", "")
                    est_val = 15000.0
                    if "-" in val_str:
                        try:
                ...
                if match_count > 0:
                    print(f"  -> {feed_name}: Extracted {match_count} political trades.")
                    
        except Exception:
            # Swallow exceptions to prevent one failed feed from breaking the scan
            pass
            
    if records:
        print(f"[+] Successfully extracted {len(records)} total political trades.")
    else:
        print(f"[-] No recent political trades found for ${ticker.upper()} in public feeds.")
