...
def fetch_open_senate_json(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    ...
    for row in data:
        ...
        # We already have a dedicated House PDF scraper, so we only parse Senate here
        ...
        # Dynamically extract lawmaker name based on the JSON schema
    ...
    if match_count > 0:
        print(f"  [+] Extracted {match_count} historical trades from active Senate database.")
    else:
        print(f"  [-] No recent political trades found in Senate database for ${ticker.upper()}.")
        
    return pd.DataFrame(records)

# =====================================================================
# 4. HOUSE PDF SCRAPER (MULTI-THREADED WITH OCR)
# =====================================================================
def fetch_local_house_pdfs(ticker: str, aliases: list, lookback_days: int = 365) -> pd.DataFrame:
    ...
    # Execute with 10 concurrent threads to blast through the 500+ PDFs
