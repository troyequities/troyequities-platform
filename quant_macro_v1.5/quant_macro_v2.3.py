...
# =====================================================================
# 3. LOCAL GOVERNMENT FILE SCRAPER (WITH OCR FALLBACK)
# =====================================================================
def fetch_local_house_pdfs(ticker: str, aliases: list, lookback_days: int = 365) -> pd.DataFrame:
    ...
    if OCR_AVAILABLE:
        print("  [>] OCR Engine: ONLINE (Ready for handwritten PDFs)")
    else:
        print("  [>] OCR Engine: OFFLINE (Skipping scanned image PDFs)")
    ...
    for idx, ptr in enumerate(ptr_list):
        ...
        if os.path.exists(pdf_path):
            try:
                ...
                # 2. OCR Fallback: If no text was extracted, it's a scanned image
                if not pdf_text.strip() and OCR_AVAILABLE:
                    try:
                        ...
                        pass # Ignore errors if Poppler/Tesseract system binaries are missing
