...
# =====================================================================
# Hardcoded Finnhub API Key for Corporate Insider Data:
...
def get_company_aliases(ticker: str) -> list:
    """
    Dynamically generates search aliases using yfinance to catch 
    handwritten or non-standard naming conventions in government PDFs.
    """
    ...
    try:
        ...
        for key in ['shortName', 'longName']:
            ...
            if name:
                aliases.append(name)
                # Extract the first prominent word (e.g., 'Apple' from 'Apple Inc.')
    ...
    # Remove duplicates and sort by length descending (longest phrases first)
    unique_aliases = list(set(aliases))
    return sorted(unique_aliases, key=len, reverse=True)

# =====================================================================
# 3. LOCAL GOVERNMENT FILE SCRAPER (ZERO API COST, MULTI-YEAR)
# =====================================================================
def fetch_local_house_pdfs(ticker: str, aliases: list, lookback_days: int = 365) -> pd.DataFrame:
    """
    Directly scrapes the US House Office of the Clerk across multiple years.
    Parses the raw PDF documents locally looking for any known alias.
    """
    print(f"[+] Initializing Direct US House PDF Scraper for ${ticker.upper()}...")
    print(f"  [>] Tracking Aliases: {', '.join(aliases)}")
    ...
    # 1. Loop through all years required by the lookback window
    for year in range(start_year, current_year + 1):
        ...
        # 2. Parse the XML for this specific year
        try:
    ...
    # 3. Download and parse PDFs locally using Regex Alias Matching
    ptr_list = sorted(ptr_list, key=lambda x: x['date'], reverse=True)
    total_ptrs = len(ptr_list)
    print(f"  [>] Found {total_ptrs} total stock trade filings across {start_year}-{current_year}. Scanning all...")
    
    # Build the massive regex pattern for all aliases
    ...
    for idx, ptr in enumerate(ptr_list):
        ...
        if os.path.exists(pdf_path):
            try:
                ...
                # Search using the dynamic alias pattern (case-insensitive)
...
# =====================================================================
# 4. CORPORATE INSIDER DATA LAYER (FINNHUB)
# =====================================================================
...
def get_unified_flow_data(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    # Generate the robust alias list
    aliases = get_company_aliases(ticker)
    
    pol_df = fetch_local_house_pdfs(ticker, aliases, lookback_days)
...
# =====================================================================
# 6. CORE ALGORITHMIC SCORING ENGINE
# =====================================================================
