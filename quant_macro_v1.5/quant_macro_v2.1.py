...
# =====================================================================
# 2. LOCAL GOVERNMENT FILE SCRAPER (ZERO API COST, MULTI-YEAR)
# =====================================================================
def fetch_local_house_pdfs(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    ...
    Downloads the official XML indexes and parses the raw PDF documents locally.
    """
    print(f"[+] Initializing Direct US House PDF Scraper for ${ticker.upper()}...")
    print("    (Note: Downloading and parsing multi-year government PDFs takes time. Please wait...)")
    ...
    for year in range(start_year, current_year + 1):
        ...
        if not xml_files:
            print(f"  [>] Downloading {year} Master XML Index from House.gov...")
            try:
                ...
                if r.status_code == 200:
                    ...
                    print(f"  [!] Failed to download {year} US House Index: HTTP {r.status_code}")
                    continue
            except Exception as e:
                print(f"  [!] Failed to download {year} US House Index: {e}")
                continue
                
            xml_files = [f for f in os.listdir(cache_dir) if f.lower().endswith('.xml')]

        if not xml_files:
            print(f"  [!] Could not locate XML index inside the downloaded ZIP for {year}.")
            continue
        ...
        try:
            ...
            print(f"  [!] Failed to parse {year} XML: {e}")
