...
# =====================================================================
# 3. SENATE ELECTRONIC FINANCIAL DISCLOSURES (eFD) SCRAPER
# =====================================================================
def fetch_local_senate_efd(ticker: str, aliases: list, lookback_days: int = 365) -> pd.DataFrame:
    """
    Bypasses the Senate eFD gateway agreement to scrape electronic PTRs.
    """
    print(f"[+] Initializing Direct US Senate eFD Scraper for ${ticker.upper()}...")
    ...
    try:
        # 1. Bypass the CSRF Agreement Gateway
        r = session.get(home_url, headers=headers, timeout=10)
        csrf_token = session.cookies.get('csrftoken', '')
        session.post(home_url, data={'prohibition_agreement': '1', 'csrfmiddlewaretoken': csrf_token}, headers={'Referer': home_url, 'User-Agent': 'Mozilla/5.0'})
        
        # 2. Search for Periodic Transaction Reports (Report Type 11)
        payload = {
            "start_date": cutoff_date.strftime("%m/%d/%Y"),
        ...
        search_res = session.post(search_url, data=payload, headers={'Referer': 'https://efdsearch.senate.gov/search/', 'User-Agent': 'Mozilla/5.0'})
        reports = search_res.json().get('data', [])
    except Exception as e:
        print(f"  [!] Senate Portal Connection Error: {e}")
        return pd.DataFrame()

    total_reports = len(reports)
    print(f"  [>] Found {total_reports} recent Senate PTR filings. Scanning HTML...")
    ...
    # 3. Read specific HTML filings
    for idx, report in enumerate(reports):
        print(f"  [~] Processing Senate Report {idx + 1} of {total_reports}...", end='\r')
        
        # report[3] contains the anchor tag with the document UUID
        match = re.search(r'href="([^"]+)"', str(report[3]))
        if not match: continue
        
        report_url = "https://efdsearch.senate.gov" + match.group(1)
        first_name = str(report[0]).strip()
        last_name = str(report[1]).strip()
        date_filed = str(report[4]).strip()

        try:
            doc_res = session.get(report_url, headers=headers, timeout=5)
            
            # Search the raw HTML for the alias
            if re.search(regex_pattern, doc_res.text, re.IGNORECASE):
                records.append({
                    "Date": datetime.strptime(date_filed, "%m/%d/%Y").strftime("%Y-%m-%d"),
                    "Entity": f"CONGRESS: {first_name} {last_name} (Senate)"[:26],
                    "Source": "Congress (Senate eFD)",
                    "Position": "BUY", # Baseline matrix assumption 
                    "Volume": "$15,000 (Est)",
                    "Est_Value": 15000.0
                })
                match_count += 1
    ...
    print(" " * 60, end='\r')
    if match_count > 0:
        print(f"  [+] Successfully located {match_count} Senate eFD disclosures for ${ticker.upper()}.")
    else:
        print(f"  [-] Checked recent Senate PTRs. No trades found for ${ticker.upper()}.")

    return pd.DataFrame(records)

# =====================================================================
# 4. HOUSE PDF SCRAPER (WITH OCR FALLBACK)
# =====================================================================
def fetch_local_house_pdfs(ticker: str, aliases: list, lookback_days: int = 365) -> pd.DataFrame:
    ...
    print(f"  [>] Found {total_ptrs} total House filings across {start_year}-{current_year}. Scanning...")
    ...
    for idx, ptr in enumerate(ptr_list):
        print(f"  [~] Processing House PDF {idx + 1} of {total_ptrs}...", end='\r')
        pdf_year = ptr['year']
    ...
    print(" " * 60, end='\r') 
    if match_count > 0: print(f"  [+] Successfully located {match_count} House PDF disclosures.")
    else: print(f"  [-] Checked all recent House PDFs. No trades found.")
    return pd.DataFrame(records)

# =====================================================================
# 5. CORPORATE INSIDER DATA (WITH 10b5-1 HEURISTIC FILTERING)
# =====================================================================
def fetch_finnhub_insider(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    ...
    try:
        ...
        # --- THE 10b5-1 HEURISTIC FILTER ---
        # If the exact same corporate entity sells the exact same amount of shares 
        # on multiple different dates, it is overwhelmingly likely a 10b5-1 automated plan.
        df['Is_10b5_1'] = False
        sell_mask = df['Position'] == 'SELL'
        
        # Identify repeating sale volumes by the same insider
        ...
        if flagged_count > 0:
            ...
            # Label them for the math engine to heavily penalize their alpha conviction
...
def get_unified_flow_data(ticker: str, lookback_days: int = 365) -> pd.DataFrame:
    ...
    senate_df = fetch_local_senate_efd(ticker, aliases, lookback_days)
...
# =====================================================================
# 7. CORE ALGORITHMIC SCORING ENGINE
# =====================================================================
def apply_alpha_scoring_math(df: pd.DataFrame) -> pd.DataFrame:
    ...
    def map_lynch_conviction(row):
        ...
        # Penalize automated 10b5-1 dumps (They convey almost zero negative conviction)
